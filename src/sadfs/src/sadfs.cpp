...
Sadfs::Sadfs(sadfsOptions *options, struct fuse_conn_info *conn, struct fuse_context *fc)
	...
	int Sadfs::getattr (const char *path, struct stat *stat) {
		...
		stat->st_mode = this->pr->getMode(rp->getPathId());        /* File type and mode */
		if (this->pr->isDirectory(rp->getPathId())) {
			stat->st_nlink = 2;       /* Number of hard links */
		} else {
			stat->st_nlink = 1;       /* Number of hard links */
		}
#if 0
		stat->st_uid = this->fuse_context->uid;         /* User ID of owner */
		stat->st_gid = this->fuse_context->gid;         /* Group ID of owner */
		stat->st_rdev;        /* Device ID (if special file) */
		stat->st_size = 0;        /* Total size, in bytes */
		stat->st_blocks = 0;      /* Number of 512B blocks allocated */

		struct timespec ts;
		clock_gettime (CLOCK_REALTIME, &ts);
		stat->st_atim = ts;  /* Time of last access */
		stat->st_mtim = ts;  /* Time of last modification */
		stat->st_ctim = ts;  /* Time of last status change */
#endif
		if (this->options->debug) fprintf(stderr, "%s mode: %d\n", __FUNCTION__, stat->st_mode);
	...
	int Sadfs::rmdir (const char *path) {
		...
		if (!this->pr->isDirectory(path_id)) {
			delete rp;
			return -ENOTDIR;
		}
